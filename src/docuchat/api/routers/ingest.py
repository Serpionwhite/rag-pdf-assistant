...
async def ingest_pdf(file: UploadFile) -> IngestResponse:
    ...
    chunks = split_documents(document, chunk_size=settings.chunk_size, chunk_overlap=settings.chunk_overlap)

    try:
        vector_store = build_vectorstore(chunks, settings.chroma_persist_dir)
    except Exception as e:
    ...
    logger.info("Ingested %s: %d pages, %d chunks", file.filename, len(document), len(chunks))


    return IngestResponse(filename=file.filename, pages=len(document), chunks=len(chunks))
